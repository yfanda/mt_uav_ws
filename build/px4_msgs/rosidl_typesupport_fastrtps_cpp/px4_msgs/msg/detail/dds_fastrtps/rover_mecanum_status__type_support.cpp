  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = px4_msgs::msg::RoverMecanumStatus;
    is_plain =
      (
      offsetof(DataType, pid_lateral_throttle_integral) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _RoverMecanumStatus__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_RoverMecanumStatus(full_bounded, is_plain, 0);
